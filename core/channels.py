...
def _row_to_dict(row, manifest=None, fallback_manifests: dict | None = None) -> dict:
    ...
    base = {
        ...
        "source_kind": getattr(row, "source_kind", "hls") or "hls",
...
class ChannelManager:
        ...
        try:
            ...
            with get_session() as session:
                ...
                if "source_kind" in data:
                    row.source_kind = data["source_kind"] or "hls"
    ...
    def remove_fallback_source(self, channel_id: str, manifest_id: str) -> dict | None:
        ...
        with get_session() as session:
            ...
            row.fallback_manifest_ids = chain
            if manifest_id in (row.fallback_encoder_modes or {}):
                modes = dict(row.fallback_encoder_modes or {})
    ...
    def delete_channel(self, channel_id: str) -> bool:
        ...
        if existing is None:
            return False
        # For tab_proxy channels, close the sidecar tab so its browser
        # resources are freed. Best-effort — sidecar may be unreachable
        # or already have no tab for this channel.
        if existing.get("encoder_mode") == "tab_proxy":
            try:
                import requests as http_requests
                from core.config import get_setting
                sidecar = get_setting("SELENIUM_URL", "http://localhost:4445")
                http_requests.delete(f"{sidecar}/tab/{channel_id}", timeout=5)
            except Exception as e:
                logging.debug("[CHANNELS] tab close on delete failed: %s", e)
        try:
