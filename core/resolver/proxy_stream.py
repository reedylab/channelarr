...
class ProxyStream:
        ...
        while not self._stop_event.is_set():
            ...
            for line in lines:
                ...
                if line.startswith("#EXT-X-KEY:"):
                    ...
                    # Extract sequence from the URI or use position
            ...
            for seg in segments:
                ...
                try:
                    ...
                    segment_files.append((local_seq, local_filename, seg["duration"]))
                    local_seq += 1
                    new_count += 1
                except Exception as e:
                    logging.warning("[PROXY] %s download failed for seq %s: %s",
            ...
            if segment_files:
                ...
                while len(segment_files) > MAX_SEGMENTS_ON_DISK:
                    _, old_file, _ = segment_files.pop(0)
            ...
            # Prune seen_seqs to avoid unbounded growth
            if len(seen_seqs) > 1000:
                min_keep = min(s["seq"] for s in segments) if segments else 0
    ...
    def _write_playlist(self, segment_files: list[tuple[int, str, float]]):
