...
def run_scraper(name: str, config: dict | None = None):
    """Execute a single scraper and feed results to the event queue or
    directly to the batch resolver, depending on `use_event_queue`."""
    config = config or {}
    default_tags = config.get("default_tags", [])
    timeout = config.get("timeout", 90)
    use_event_queue = config.get("use_event_queue", True)
    title_filter_raw = (config.get("title_filter") or "").strip()
    title_filters = [s.strip().lower() for s in title_filter_raw.split(",") if s.strip()]
    title_filter_invert = bool(config.get("title_filter_invert", False))

    with _run_lock:
        _running.add(name)
        logger.info("[SCRAPER] Running scraper: %s (use_event_queue=%s)", name, use_event_queue)
        ...
        try:
            ...
            # Optional filter (case-insensitive substring match). Comma-separated
            # terms; each term is tested against the event's title AND its
            # tags. Non-invert (default): keep only events that match. Invert:
            # drop events that match. Applied before dedup so filtered-out
            # events never enter the queue.
            if title_filters:
                before = len(events)
                def _matches(e):
                    hay = [(e.get("title") or "").lower()]
                    hay.extend(str(t).lower() for t in (e.get("tags") or []))
                    return any(t in h for t in title_filters for h in hay)
                if title_filter_invert:
                    events = [e for e in events if not _matches(e)]
                else:
                    events = [e for e in events if _matches(e)]
                logger.info("[SCRAPER] %s: filter %s (invert=%s) → %d/%d events pass",
                            name, title_filters, title_filter_invert, len(events), before)
            ...
            if use_event_queue:
                from core.event_resolver import upsert_events
                logger.info("[SCRAPER] %s: upserting %d events into scraped_events queue",
                            name, len(events))
                upsert_events(name, events)
            else:
                # Legacy passthrough: feed straight to the batch resolver.
...
def get_status() -> dict:
    ...
    for name in set(list(config.get("scrapers", {}).keys()) + available):
        ...
        scrapers[name] = {
            ...
            "use_event_queue": cfg.get("use_event_queue", True),
            "title_filter": cfg.get("title_filter", ""),
            "title_filter_invert": cfg.get("title_filter_invert", False),
...
def _migrate_use_event_queue_flag():
    """Ensure every scraper config entry has `use_event_queue` set (default true).

    Idempotent; writes SCRAPER_CONFIG only if any entry was missing the flag.
    """
    import json
    ...
    for name, cfg in scrapers.items():
        if "use_event_queue" not in cfg:
            cfg["use_event_queue"] = True
            changed = True
    if changed:
        config["scrapers"] = scrapers
        save_settings({"SCRAPER_CONFIG": json.dumps(config)})
        logger.info("[SCRAPER] Migrated SCRAPER_CONFIG: added use_event_queue=true to %d entries",
                    sum(1 for c in scrapers.values() if c.get("use_event_queue")))
