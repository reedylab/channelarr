...
from core.nfo import read_nfo_plot, read_nfo_title, find_poster

WINDOW_HOURS = 48


def _enrich_entry(entry: dict, base_url: str) -> tuple[str, str, str | None]:
    """Pull live NFO title / plot / poster for a materialized schedule entry.

    The materialized_schedule snapshots desc/title at channel-create time
    and never revisits them, so NFOs added AFTER channel creation never
    reach the EPG. Thumbnails were never cached at all for local content.
    Resolve all three live here so the guide always reflects current NFO
    state without needing a re-materialize.

    Returns (title, desc, thumbnail_url_or_None). Falls back to whatever
    the materialized entry has for title/desc; poster is None if no image
    lives next to the file.
    """
    path = entry.get("path") or ""
    etype = entry.get("type", "")

    # YouTube entries carry their own title/thumbnail from the YT metadata;
    # don't touch them. Bumps never reach this point (filtered earlier).
    if etype == "youtube":
        return entry.get("title", ""), entry.get("desc", ""), entry.get("thumbnail") or None
    ...
    if path and os.path.isfile(path):
        try:
            live_title = read_nfo_title(path)
            if live_title:
                title = live_title
        except Exception as e:
            logging.debug("[XMLTV] NFO title read failed for %s: %s", path, e)
        try:
            live_desc = read_nfo_plot(path)
            if live_desc:
                desc = live_desc
        except Exception as e:
            logging.debug("[XMLTV] NFO plot read failed for %s: %s", path, e)
        try:
            poster = find_poster(path)
            if poster:
                thumb_url = f"{base_url}/api/media/poster?path={quote(path)}"
        except Exception as e:
            logging.debug("[XMLTV] Poster lookup failed for %s: %s", path, e)

    return title, desc, thumb_url
...
def generate_channelarr_xmltv(channels: list, output_path: str, base_url: str):
    ...
    for ch in channels:
        ...
        for entry in content_entries:
            ...
            title, desc, thumb_url = _enrich_entry(entry, base_url)
            ...
            if thumb_url:
                SubElement(prog, "icon", src=thumb_url)
            else:
                logo_url = f"{base_url}/api/logo/{cid}"
